...
| **repro** | `tools/repro/`: `repro.ps1` puts this box into a named configuration a bug report was made in (`profiles/<name>/profile.json`), launches the game into it, judges each launch by `.claude/rules/in-game-verification.md` alone and restores the install byte for byte; `fingerprint.ps1` is the independent check that it did. Payloads, vaults, snapshots and run manifests live outside the repo, in `WUCHANG_REPRO_STORE`. `-Probe present` adds the two frame instruments to the same runner — PresentMon split by `SwapChainAddress`, and the mod's own frame census — and needs the right to open an ETW session (administrator, or Performance Log Users). See `tools/repro/README.md` |
...
| elite, bamboozling | SLAIN — zero health, the same read. These two, the boss and the cuckoo are what `mdb::slain_is_found()` names: finite collections of characters rather than mobs, marked permanently on the first kill - the cuckoo by the player's own hand, since nothing here can read it. An elite's pawn class is an ordinary AI-possessed enemy at runtime, so the *static twin* joined on the id is what says it was the tougher variant |
| cuckoo | MARKED BY HAND on the full map, and it is the one whose death the live sweep cannot watch: a `DSCActor` (`BP_NoActionAI_C`) with no AI controller and no pawn, so `Rule::ControllerPawn` and `Rule::PawnHealth` do not apply, no `kClasses` row names it and no absence rule covers it; its 105 markers are drawn from the static data alone. It is also the one category that **comes back** — resting respawns it — so the mark counts towards the 105 while `mdb::survives_being_found()` keeps the marker on the map under `markers_hide_found` and in the x-ray |
