...
//
// The game is stock UE 5.1 Enhanced Input with no user-settings object, so a remap in
// the options menu is visible in exactly one place: the flattened
// `UEnhancedPlayerInput::EnhancedActionMappings` array on the local player's
// `PlayerInput`. The shipped `UInputMappingContext` assets keep the defaults, so they
// are not read.
//
...
//
// Every offset comes from reflection: the array property's offset from the class walk,
// the element stride from the `UScriptStruct`'s `GetPropertiesSize()`, and `Action` /
// `Key` / `KeyName` from walking that struct. Nothing is hand-mirrored, and the derived
// layout is logged and validated against the array's own span before a single row is
// read. `FKey` is not eight bytes - it caches a `TSharedPtr<FKeyDetails>` - so only the
// `FName` inside it is touched.
//
// The array is rebuilt on every remap and the `UEnhancedPlayerInput` is recreated with
// the controller, so nothing is cached by index and the whole chain is re-resolved when
// the object dies. Rows repeat in both directions: one action can hold several keys and
// one key several actions, which is why the panel's question is only ever "does this key
// appear at all".
//
// THREADS
//   game thread   game_thread_pump(), drop_caches()  - raw reads and GetName() only
//   any thread    table(), generation()              - spinlocked copy / atomic
//
